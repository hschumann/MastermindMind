...
while True:
	...
	if 20 < len(all_words) < 70:
		counts = []
		for x in range(len(all_words)):
			count = 0
			first_word = list(all_words[x])
			for y in range(len(all_words)):
				second_word = list(all_words[y])
				for l in first_word:
					if l in second_word:
						count += 1
			counts.append(count)
		m = max(counts)
		bests = [i for i, j in enumerate(counts) if j == m]
		index = bests[random.randrange(len(bests))]
